...
# --------------------------
def load_label_map():
    label_map_path = os.path.join(os.path.dirname(__file__), "../ml/registry/label_map.yml")
...
def get_valid_model_path():
    ...
    current_model_file = os.path.join(os.path.dirname(__file__), "../ml/registry/current_model.md")
    candidate_paths = []
...
def predict():
    try:
        ...
        if missing_params:
            return jsonify({"status": "fail", "error": f"缺少参数：{', '.join(missing_params)}"}), 400
